...
void loop() {
  ...
  if ( h > 48 )
  {
    strip.setPixelColor(1, strip.Color(255, 0, 0));   // включаем красный цвет на 1 светодиоде
    strip.show();   // отправляем сигнал на ленту
  }
  if (h <= 48 & h != 0)
  {
    strip.setPixelColor(1, strip.Color(0, 255, 0));   // включаем зеленый цвет на 1 светодиоде
    strip.show();   // отправляем сигнал на ленту
  }
  if (d > 740 )
  {
    strip.setPixelColor(2, strip.Color(255, 0, 0));   // включаем красный цвет на 1 светодиоде
    strip.show();   // отправляем сигнал на ленту
  }
  if (d <= 740 & d != !bme.begin(0x76))
  {
    strip.setPixelColor(2, strip.Color(0, 255, 0));   // включаем зеленый цвет на 1 светодиоде
    strip.show();   // отправляем сигнал на ленту
    lcd.setCursor(8, 1);
    lcd.print("SVET:");
  }
